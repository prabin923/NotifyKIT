...
@Module({
  imports: [
    ConfigModule.forRoot({ isGlobal: true, validationSchema: Joi.object({ NODE_ENV: Joi.string().valid('development', 'test', 'production').default('development'), DATABASE_URL: Joi.string().uri({ scheme: ['postgresql', 'postgres', 'prisma', 'prisma+postgres'] }).required(), REDIS_URL: Joi.string().uri({ scheme: ['redis', 'rediss'] }).required(), JWT_SECRET: Joi.string().min(32).required(), JWT_EXPIRES_IN: Joi.string().default('15m'), API_KEY_PEPPER: Joi.string().min(24).required(), WEBHOOK_ENCRYPTION_KEY: Joi.string().required(), API_PORT: Joi.number().port().default(3000), DASHBOARD_URL: Joi.string().uri().default('http://localhost:3001'), CORS_ORIGINS: Joi.string().default('http://localhost:3001'), EMAIL_PROVIDER: Joi.string().valid('console', 'smtp').default('console'), PUSH_PROVIDER: Joi.string().valid('console', 'fcm').default('console') }).unknown(true) }),
