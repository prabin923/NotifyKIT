...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  const pathname = usePathname(); const router = useRouter(); const [email, setEmail] = useState(''); const [service, setService] = useState<'checking' | 'ready' | 'unavailable'>('checking');
  useEffect(() => {
    const identity = window.localStorage.getItem('notification-dashboard-user');
    if (!identity) return;
    try {
      const parsed: unknown = JSON.parse(identity);
      if (parsed && typeof parsed === 'object' && 'email' in parsed && typeof parsed.email === 'string') setEmail(parsed.email);
    } catch {
  ...
  useEffect(() => {
    ...
    void fetch(`${apiUrl}/health/ready`, { cache: 'no-store' })
      .then((response) => { if (active) setService(response.ok ? 'ready' : 'unavailable'); })
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[250px_1fr]">
    ...
    <main className="min-w-0"><header className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4"><div><p className="text-sm text-slate-500">Operations console</p><h1 className="text-lg font-semibold capitalize">{pathname === '/' ? 'Dashboard' : pathname.slice(1).replace('-', ' ')}</h1></div><span className={`rounded-full px-3 py-1 text-xs font-medium ${service === 'ready' ? 'bg-emerald-50 text-emerald-700' : service === 'unavailable' ? 'bg-rose-50 text-rose-700' : 'bg-amber-50 text-amber-700'}`}>{service === 'ready' ? 'Systems operational' : service === 'unavailable' ? 'API unavailable' : 'Checking systems…'}</span></header><div className="p-5 lg:p-8">{children}</div></main>
  </div>;
}
